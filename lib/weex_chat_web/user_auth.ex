defmodule WeexChatWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_weex_chat_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      WeexChatWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule WeexChatWeb.PageLive do
        use WeexChatWeb, :live_view

        on_mount {WeexChatWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{WeexChatWeb.UserAuth, :ensure_authenticated}] do
  ...
  def on_mount(:require_user_owns_message, %{"id" => message_id}, _session, socket) do
    message = WeexChat.Chat.get_message!(message_id)
